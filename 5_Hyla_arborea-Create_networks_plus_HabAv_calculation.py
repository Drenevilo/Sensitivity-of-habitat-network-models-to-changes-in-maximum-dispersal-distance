#-------------------------------------------------------------------------------
# Script name:       Hyla_arborea-Create_networks_plus_HabAv_calculation
# Purpose:           Edge definition (Generation of networks) + calculation of Habitat availability predictor
...
# 1st version created:     31/05/2018
#-------------------------------------------------------------------------------
...
# Input settings
wrkspc = "E:\\PhD\\Automated_Run_BioCHECNET\\NoPseudorep\\Networks\\" #Where the network files are created
inputFiles = "E:\\PhD\\Automated_Run_BioCHECNET\\NoPseudorep\\Network_setup\\" #Where the cost surfaces are sourced from
costSurface = inputFiles+"UniformCS.tif" #UniformCS = UniformCostSurface, species independent
run_name = "uniform" #The name that will be given to the output files.
...
patchCodeRast = "E:\\PhD\\Automated_Run_BioCHECNET\\NoPseudorep\\Network_setup\\habPatchCode_NoPseudorep.tif"
habPatchCode = Raster(patchCodeRast)

arcpy.RasterToPolygon_conversion(habPatchCode, "habPatchCode_NoPseudorep.shp", "NO_SIMPLIFY", "VALUE") #Change raster to polygon
arcpy.Dissolve_management("habPatchCode_NoPseudorep.shp", "habPatchCode_NoPseudorep_Diss.shp", "GRIDCODE", "", "MULTI_PART", "") #Disolve the polygons based on the patch code
arcpy.FeatureToPoint_management("habPatchCode_NoPseudorep_Diss.shp", "habPatchCode_NoPseudorep_Cntrd.shp", "INSIDE") #Find the centroid of all the patches.
arcpy.AddXY_management ("habPatchCode_NoPseudorep_Cntrd.shp") #Add the X and Y coordinates of each centroid to the attribute table
...
with arcpy.da.SearchCursor("E:\\PhD\\Automated_Run_BioCHECNET\\NoPseudorep\\Networks\\habPatchCode_NoPseudorep_Cntrd.shp", fields) as cursor:
    for row in cursor:
...
##                if rPatch < 21: #Temporary restriction
...
# Set the python working directory
wrkspc = "E:\\PhD\\Automated_Run_BioCHECNET\\NoPseudorep\\Networks"
os.chdir(wrkspc)
# Setting the initial environment
env.workspace = wrkspc
arcpy.SetProduct('ArcInfo')
...
H_CDgraph = ig.Graph.Read_GraphML("uniform.graphml")
...
np.savetxt('uniform_habAv.txt', habAv, fmt='%.10f')
