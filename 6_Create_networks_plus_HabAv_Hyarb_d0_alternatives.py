#-------------------------------------------------------------------------------
# Name:         Create_networks_plus_HabAv_Hyarb_d0_alternatives
...
#
# 1st version created:     02-11-2021
...
def edge_table_to_igraph(patch_table, edge_table, xy_names, patch_dict = -1, capacity_col = -1, weight_col = -1, probability_col = -1):
    ...
    Requires:
        ...
        patch_dict: (optional) A dictionary with values that should be assigned to the nodes (NEEDS TO BE UPDATED)
    ...
    if patch_dict != -1:
        G.vs['patch'] = [patch_dict.get(int(enn), [])[0] for enn in G.vs['name']] # The number of the patch to which a node belongs
        G.vs['edge_bool'] = [patch_dict.get(int(enn), [])[1] for enn in G.vs['name']] # Boolean indicator whether an edge is on the edge of a patch or on the edge of a gap
...
costSurface = inputFiles+"UniformCS.tif" # here could go different cost surfaces
run_name = "Hyarb_6KmDispDist" #The name that will be given to the output files
...
# the above function
d0 = 452 # Median dispersal distance
# In order to speed up the calculations a maximum cost-distance can be indicated in the arcpy cost-distance
# function. This maximum cost-distance is the distance beyond which the probability of dispersal drops below
...
# [Done]            6Km:    d0 = 452 -> Max.Disp.dist. = 6006.045996    #The last one to be processed, which is why this distance is in the script
# [Done]            8Km:    d0 = 602 -> Max.Disp.dist. = 7999.202852
# [Done]            10Km:   d0 = 753 -> Max.Disp.dist. = 10005.64742
...
patchCodeRast = "C:\\Users\\damiano\\Documents\\PhD\\Additional_species_runs\\Network_setup\\habPatchCode_NoPseudorep.tif"
habPatchCode = Raster(patchCodeRast)

arcpy.RasterToPolygon_conversion(habPatchCode, "habPatchCode_Hyarb_6KmDispDist.shp", "NO_SIMPLIFY", "VALUE") #Change raster to polygon
arcpy.Dissolve_management("habPatchCode_Hyarb_6KmDispDist.shp", "habPatchCode_Hyarb_6KmDispDist_Diss.shp", "GRIDCODE", "", "MULTI_PART", "") #Disolve the polygons based on the patch code
arcpy.FeatureToPoint_management("habPatchCode_Hyarb_6KmDispDist_Diss.shp", "habPatchCode_Hyarb_6KmDispDist_Cntrd.shp", "INSIDE") #Find the centroid of all the patches.
arcpy.AddXY_management ("habPatchCode_Hyarb_6KmDispDist_Cntrd.shp") #Add the X and Y coordinates of each centroid to the attribute table
NAhabPatchCode = int(habPatchCode.noDataValue) #Determine the NoData value

#Convert the habPatchCode to numpy array
PC = arcpy.RasterToNumPyArray(habPatchCode) #PC includes the patch numbers
...
with arcpy.da.SearchCursor("C:\\Users\\damiano\\Documents\\PhD\\Sensitivity_DispDist\\Networks\\habPatchCode_Hyarb_6KmDispDist_Cntrd.shp", fields) as cursor:
    for row in cursor:
...
H_CDgraph = ig.Graph.Read_GraphML("Hyarb_6KmDispDist.graphml")
habPatchCode = "C:\\Users\\damiano\\Documents\\PhD\\Additional_species_runs\\Network_setup\\habPatchCode_NoPseudorep.tif" #change for species names or codes
...
np.savetxt('Hyarb_6KmDispDist_habAv.txt', habAv, fmt='%.10f')
