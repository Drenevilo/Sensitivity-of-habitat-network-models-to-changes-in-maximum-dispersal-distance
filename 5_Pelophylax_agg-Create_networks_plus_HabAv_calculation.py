#-------------------------------------------------------------------------------
# Name:         Pelophylax_agg-Create_networks_plus_HabAv_calculation
...
# Input settings
wrkspc = "C:\\Users\\damiano\\Documents\\PhD\\Additional_species_runs\\Networks\\" #Where the network files are created
inputFiles = "C:\\Users\\damiano\\Documents\\PhD\\Additional_species_runs\\Network_setup\\" #Where the cost surfaces are sourced from
costSurface = inputFiles+"UniformCS.tif" #UniformCostSurface, species independent
run_name = "Peagg" #The name that will be given to the output files.
...
# the above function
d0 = 133 # Median dispersal distance, specify for each species depending on reported max. disp. dist., corresponding to max. cost distance in the following explanation:
...
patchCodeRast = "C:\\Users\\damiano\\Documents\\PhD\\Additional_species_runs\\Network_setup\\habPatchCode_Peagg1.tif"
habPatchCode = Raster(patchCodeRast)

arcpy.RasterToPolygon_conversion(habPatchCode, "habPatchCode_Peagg1.shp", "NO_SIMPLIFY", "VALUE") #Change raster to polygon
arcpy.Dissolve_management("habPatchCode_Peagg1.shp", "habPatchCode_Peagg1_Diss.shp", "GRIDCODE", "", "MULTI_PART", "") #Disolve the polygons based on the patch code
arcpy.FeatureToPoint_management("habPatchCode_Peagg1_Diss.shp", "habPatchCode_Peagg1_Cntrd.shp", "INSIDE") #Find the centroid of all the patches.
arcpy.AddXY_management ("habPatchCode_Peagg1_Cntrd.shp") #Add the X and Y coordinates of each centroid to the attribute table
...
with arcpy.da.SearchCursor("C:\\Users\\damiano\\Documents\\PhD\\Additional_species_runs\\Networks\\habPatchCode_Peagg1_Cntrd.shp", fields) as cursor:
    for row in cursor:
...
# Set the python working directory
wrkspc = "C:\\Users\\damiano\\Documents\\PhD\\Additional_species_runs\\Networks\\"
os.chdir(wrkspc)
# Setting the initial environment
env.workspace = wrkspc
arcpy.SetProduct('ArcInfo')
...
H_CDgraph = ig.Graph.Read_GraphML("Peagg.graphml")
...
##patch_pop = (suit_tab['popPatch']/(np.pi*popRad*popRad))*10000 #Vector of average population per hectrare living around a patch at radius popRad: np.pi*popRad^2
## To calculate suitability the patch_size is multiplied by the negative exponential funtion of patch_pop. The patch_pop value is devided by 10 in order to obtain habitat suitability values that are within an acceptable range.
...
neighbors = H_CDgraph.neighborhood(vertices=None, order=2, mode="all") #Identify all the neighbors that can be reached in "X-order" number of steps.
...
np.savetxt('Peagg_habAv.txt', habAv, fmt='%.10f')
